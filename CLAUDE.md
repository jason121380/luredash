...
**LINE push report button** (`campaign_line_push_configs.report_variant`, `'standard'|'perf'`, `_norm_report_variant` whitelists): when `include_report_button` is on, `GroupPushConfigModal` shows a dropdown (以廣告組合報告 / 以廣告報告). The button links to a frozen snapshot (`/r/s/:id`, `_snapshot_url_for_push` with the matching `variant`) generated once at send time in `_build_flex_for_config` — no live FB re-fetch per view. The snapshot's reporting window is **concretized to ISO from/to** before freezing (a rolling preset like `last_30d` would make the displayed 資料區間 drift at view time while the data stays frozen). Generation runs under a `snapshot_timeout` sub-budget (測試 endpoint passes 12s inside its 25s gateway window; scheduler default 60s) — on timeout/error the button falls back to the live `/r/:campaignId` link (`_share_url_for_config`, `?report=perf` for perf) so the push always goes out. The cancel path cleans up the snapshot shell row (`except BaseException`).
