...
FB_INSIGHTS_MIN_GAP_MS — insights FB 呼叫「起跑」之間的最小間隔 (default 150ms,`_insights_pace()` 全域鎖)。semaphore 限**並發**(擋 burst),此值限**速率**(擋持續高頻)—— 因為 `1504022` 是「一段時間內 insights 呼叫太多」,快速完成的 burst 即使並發低仍可能累積超標。150ms ≈ 全 App 最多 ~6-7 insights/秒。設 0 停用。**注意:pacing/快取/降並發都是「壓低呼叫量」,若量真的大到仍常爆,治本是向 FB 申請更高存取層級(Advanced Access,非程式)。**
...
- **FB 呼叫量治理(五層降載,避免 App 全域 `X-App-Usage` / `code=4` 每小時呼叫額度爆桶)** — attack raw FB call count from 5 complementary angles:
  ...
  3. **歷史區間長 TTL** — `_is_closed_date_range()` / `_insights_cache_ttl()`; closed windows cached `FB_HISTORICAL_CACHE_TTL_SECONDS` (3600s), rolling windows stay 300s. Applied in the 3 insights helpers.
  4. **離峰預熱** — `_is_offpeak_now()` gates the two monthly re-warm fan-outs to `FB_OFFPEAK_WARM_START_HOUR`–`END_HOUR` (2–6). Manual 工程模式 warms unaffected.
  5. **排程推播解鎖** — `_scheduler_tick()` runs UNCONDITIONALLY (NOT behind `_background_gate_reason()`); see the LINE Push Scheduler note. Only the monthly history-warm/cost-center auto-ticks stay behind the background gate.
