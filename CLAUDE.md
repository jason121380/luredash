...
- **Storage (as of 2026-04-17)**:
  - **PostgreSQL** (via `asyncpg`, `DATABASE_URL` env) — source of truth for:
    ...
    - `user_settings` (fb_user_id, key, value JSONB) — per-user: `selected_accounts`, `account_order`, **`focus_campaigns`** (array of `{id, accountId, name}` — 儀表板「重點關注」加星的行銷活動,**每人各自一份**;左側帳戶清單 + 手機選單固定第一個「重點關注」,選了就跨帳戶只顯示自己加星的活動,star 按鈕在活動列 No. 旁。`useFinanceStore.focusCampaigns`/`toggleFocus`(雖在 finance store,但走 `setUser`/`_focusUserId` 存 per-user,不是 shared),`DashboardView` 用 sentinel account id `__focus__` 切換,StatsGrid 在 focus 模式改用加星活動加總的合成 insights)
    ...
    - `fb_throttle_events` (id, ts, scope 'account'|'global', account_id, path, error_code, source, fb_user_id, bucu_pct, **app_usage_pct**, **error_subcode**) — **durable** FB rate-limit / throttle log for 工程模式「FB 限流戰情室」(`/api/engineering/fb-calls`). Written fire-and-forget (`_spawn_bg(_persist_throttle_event(...))`) from `_record_account_throttle` (per-account 80000-80014) and `_record_global_throttle` (global 4/17/32/613), capturing WHO (`_current_fb_user_id`) + WHAT page (`_fb_call_source`) + BUCU% at the moment of the hit. The in-memory ring buffers (`_account_throttle_events`/`_global_throttle_events`) are still the restart-lossy fast path / fallback; this table is the complete history (survives restart, not limited to the 5-min window). The endpoint returns `throttle_events` (newest-first, up to 200, DB-backed) + `throttle_total`, and **every table (`recent`, `top_sources_5m`, `top_accounts_5m`, `top_paths_5m`, `throttle_events`) carries the triggering `fb_user_id` + a resolved display name** (`_fb_user_display_names` batch-resolves via `fb_user_profiles`). The frontend `EngineeringView` marks the **newest** throttle event (`throttle_events[0]`) in strong red as「最後爆」. `_log_fb_call` now also records `fb_user_id`. **工程模式 has a static「限流・推播說明」tab** (`EngineeringTab` `"docs"` → `RateLimitDocsPanel`, between「各帳戶 FB 限流」and「歷史資料預熱」) explaining the three FB buckets / self-protection gates / five-layer load-reduction / scheduler behavior / how to read the throttle log — so an operator can interpret 戰情室 without reading code. **KEY: the `BUCU %` shown next to each throttle event is `_peak_bucu_pct()` (the per-account business-use-case bucket) — it is NOT the metric that governs a `code=4` (App-global) event (`X-App-Usage`), so a low BUCU next to a code-4 row is expected, not contradictory.** The recorders (`_record_account_throttle` / `_record_global_throttle`) now ALSO capture `app_usage_pct = _peak_app_usage_pct_or_none()` (max of `X-App-Usage` call_count/total_time/total_cputime — the bucket that DOES govern code 4); the log renders it as `App N%` next to `BUCU N%`, **red-bold on code-4 rows**. `_peak_app_usage_pct_or_none()` returns **None (→ NULL → `App n/a` in the panel)** when there was no fresh X-App-Usage reading on the throttle response — distinct from a genuine `App 0%`. A code-4 with `App n/a` points to a short **burst** (many `/insights` in one instant) tripping FB's spike protection, not the hourly budget being saturated — which is why the report/share page funnels its per-adset breakdown + ads fetches through `frontend/src/lib/fbConcurrency.ts` `limitFb()` (FIFO semaphore, max 4) so auto-expanding N adsets staggers instead of firing an N-wide burst. The recorders ALSO capture FB's `error_subcode` (rendered as `code=4·<subcode>` in the panel) — the field that names WHICH limit fired: the same `code=4` covers the app-hourly budget AND the ads-**insights-specific** throttle (which fires on rapid/concurrent insights calls across ad accounts regardless of `X-App-Usage`, so a code-4 with `App 0%`/`n/a` is usually THIS, not the hourly budget). Read the subcode instead of guessing from App%.
...
FB_INSIGHTS_MIN_GAP_MS — insights FB 呼叫「起跑」之間的最小間隔 (default 500ms,`_insights_pace()` 全域鎖)。semaphore 限**並發**(擋 burst),此值限**速率**(擋持續高頻)—— 因為 `1504022` 是「一段時間內 insights 呼叫太多」,快速完成的 burst 即使並發低仍可能累積超標。500ms ≈ 全 App 最多 ~2 insights/秒。設 0 停用。**注意:pacing/快取/降並發都是「壓低呼叫量」,若量真的大到仍常爆,治本是向 FB 申請更高存取層級(Advanced Access,非程式)。**
OVERVIEW_ACCOUNT_CONCURRENCY — 儀表板 overview 每次 fan-out 的帳號並發 (default 2,2026-08-04 從 4 降;儀表板載入是 1504022 的最大觸發源)。
FB_SHARED_INSIGHTS_CACHE — insights GET 讀取快取是否跨使用者共用 (default "1"=on)。`_cache_key(..., shared=True)` 用固定 `shared` 取代 per-token hash,讓 N 位同事看同批帳號時同一份 insights 只打一次 FB(其餘吃快取 + single-flight 合併並發 miss)。**單一代理商全員共用帳號時安全**(LURE);若改成多租戶(不同代理商同站),設 `0`,否則不同租戶會共用 insights bytes。`_cache_invalidate` 以 path 比對(非 token 前綴)所以 mutation 仍正確清除共用項。套在 campaign/adset/ad helpers、account-level `act_xxx/insights`、以及 `/api/breakdown`。
```
...
- **FB 呼叫量治理(多層降載,避免 App 全域 `X-App-Usage` / Ads Insights `code=4·1504022` 爆桶)** — attack raw FB call count from complementary angles:
  1. **Per-account 歸戶** — `_entity_account_map` (60min TTL) + `_learn_account_from_graph_response` / `_account_id_for_path`: entity-scoped paths (`{adset}/ads`, `{campaign}/insights`, creative ids) that carry NO `act_` prefix used to bypass the per-account throttle gate + `_account_semaphore`; now their account is learned from graph responses so they get gated too. Covered by `tests/test_rate_limit_helpers.py`.
  2. **精準 invalidate** (frontend `useEntityMutations`) — a status/budget mutation invalidates ONLY the affected account's overview query (`invalidateAffectedOverview(qc, accountId)`), not all 80. Row components thread `accountId` (optional → falls back to global invalidate).
  3. **歷史區間長 TTL + 跨使用者 shared insights cache** — `_is_closed_date_range()` / `_insights_cache_ttl()`; closed windows cached `FB_HISTORICAL_CACHE_TTL_SECONDS` (3600s), rolling windows stay 300s. Applied to campaign/adset/ad insights helpers, account-level `_fetch_account_insights`, and `/api/breakdown`.
  4. **近期 overview snapshot 不自動 full refetch** — `useMultiAccountOverview` keeps the 24h localStorage snapshot for instant paint, but a snapshot saved within 15min disables the full live `/api/overview` background refetch. This turns hard refresh / app reopen into a local read instead of another multi-account insights fan-out.
  5. **Ads Insights 全域 pacing** — `_insights_semaphore` limits concurrent `/insights` calls, and `_insights_pace()` now defaults to 500ms between starts (`FB_INSIGHTS_MIN_GAP_MS`), about 2 insights/sec app-wide, targeting `code=4·1504022`.
  6. **離峰預熱** — `_is_offpeak_now()` gates the two monthly re-warm fan-outs to `FB_OFFPEAK_WARM_START_HOUR`–`END_HOUR` (2–6). Manual 工程模式 warms unaffected.
  7. **排程推播解鎖** — `_scheduler_tick()` runs UNCONDITIONALLY (NOT behind `_background_gate_reason()`); see the LINE Push Scheduler note. Only the monthly history-warm/cost-center auto-ticks stay behind the background gate.
- **The LINE-push scheduler tick is NOT gated by `_background_gate_reason()`** (2026-07-31). Gating it there caused scheduled pushes due at e.g. 09:00 to be silently skipped (and `next_run_at` never advanced) whenever background BUCU was high. The tick now claims due configs **one at a time** (`_claim_due_line_push_config`, `LIMIT 1 FOR UPDATE SKIP LOCKED`, up to `_LINE_PUSH_MAX_CONFIGS_PER_TICK`=10/tick) so there is no "claimed-but-not-processed" stranded batch; per-call live/per-account throttles inside `fb_get` still apply, and a throttled build just fails → retry (10min×fail_count, auto-disable at 5).
...
- **KPI source (all 3 levels)**: `_build_flex_for_config` fetches metadata and insights **separately, in parallel** at every level. Numbers come from `_fetch_single_entity_insights` which hits the entity's `/insights` EDGE (`GET /{id}/insights`) — level-agnostic, so it works for campaign / adset / ad ids alike, and matches the canonical path the dashboard uses at campaign level (`act_xxx/insights?level=campaign`). Do NOT field-expand `insights` on an entity node (`?fields=insights.date_preset(X){...}`) — node expansion returned an EMPTY insights row for some entities (awareness objectives / certain delivery structures) even when they clearly spent, causing 花費 $0 on the LINE card while the dashboard showed real spend. The campaign-level single bubble uses `_fetch_single_entity_insights` (edge). The **per-member carousel** (adset/ad `以廣告組合/廣告播報`) is **BULK-fetched (2026-07-24)**: ONE `{campaign}/{kind}s` metadata call + ONE `_fetch_child_insights_bulk(campaign, kind)` insights call for ALL members, then filtered to the config's selected `member_ids` — replacing the old per-member `_fetch_member` fan-out (2 FB calls × N members). A 5-bubble carousel dropped from ~10 FB calls to 2; the fan-out was the top source of the FB global (code 4) rate limit. Same `/insights` EDGE numbers, just batched. Tiered field fallback + rate-limit re-raise mirror `_fetch_campaign_insights_bulk`.
